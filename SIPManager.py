...
def format_uri(uri, default_domain, replace_plus = None):
...
class SIPManager(object):
    ...
    def parse_sip_uri(self, target_uri, account):
        ...
        target_uri = format_uri(target_uri, account.id.domain if not isinstance(account, BonjourAccount) else None, account.pstn.replace_plus if not isinstance(account, BonjourAccount) else None)
