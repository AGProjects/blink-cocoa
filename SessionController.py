...
class SessionController(NSObject):
    ...
    def setRoutesFailed(self, msg):
        ...
        log_data = TimestampedNotificationData(direction='outgoing', target_uri=format_identity(self.target_uri, check_contact=True), timestamp=datetime.now(), code=478, failure_reason='DNS Lookup Failed', streams=self.streams_log, focus=self.remote_focus_log, participants=self.participants_log)
    ...
    def setRoutesResolved(self, routes):
        ...
        if len(routes) == 0:
            ...
            log_data = TimestampedNotificationData(direction='outgoing', target_uri=format_identity(self.target_uri, check_contact=True), timestamp=datetime.now(), code=478, failure_reason='No route found to SIP proxy', streams=self.streams_log, focus=self.remote_focus_log, participants=self.participants_log)
            self.notification_center.post_notification("BlinkSessionDidFail", sender=self, data=log_data)
    ...
    def _NH_SIPSessionDidFail(self, sender, data):
        ...
        log_data = TimestampedNotificationData(direction=session.direction, target_uri=format_identity(self.target_uri, check_contact=True), timestamp=data.timestamp, code=data.code, failure_reason=data.failure_reason, streams=self.streams_log, focus=self.remote_focus_log, participants=self.participants_log)
        self.notification_center.post_notification("BlinkSessionDidFail", sender=self, data=log_data)
